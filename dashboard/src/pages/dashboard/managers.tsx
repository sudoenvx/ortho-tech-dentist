...
function ManagersPage() {
  ...
  const openNewManagerModal = () => {
    ...
    setFormState({ name: '', email: '', password: '', role: 'viewer' })
    setIsModalOpen(true)
  }

  const openEditManagerModal = (manager: Manager) => {
    setSelectedManager(manager)
    setFormState({ name: manager.name, email: manager.email, password: manager.password, role: manager.role })
    setIsModalOpen(true)
  }
        ...
        setManagers((prev) =>
          prev.map((manager) =>
            manager.id === selectedManager.id
              ? { ...manager, name: trimmedName, email: trimmedEmail, password: trimmedPassword, role: formState.role, updatedAt: new Date() }
              : manager,
          ),
        )
      } else {
